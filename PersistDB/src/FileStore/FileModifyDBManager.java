...
private static ConcurrentLinkedQueue<DataDeleteIndex> queue=new ConcurrentLinkedQueue<DataDeleteIndex>();
public static void addDataDeleteIndex(DataDeleteIndex index)
{
    queue.offer(index);
    indexSzie++;
    if(indexSzie>100)
...
public static void addFile(DataDeleteIndex index)
{
    ...
    cachedThreadPool.execute(new Runnable() {
        ...
        StringBuffer buf=new StringBuffer();
         while(isRuning)
         {
             ...
             DataDeleteIndex tmp=queue.poll();
            buf.setLength(0);
             if(tmp==null)
             {
                 isRuning=false;
